...
import {InputPhone1} from '../../../components/Input/Input';
...
const AccountInformation = ({
  ...
  //     setShowAlert(true);
  //   }else if(from=="2"){
  //     setMsg("Something went wrong.Pls try later");
  //     setShowAlert(true);
  //   }
  // }, []);
  ...
  const aboutdetail = async id => {
    ...
    try {
      ...
      if (id == 5) {
        ...
        formData.append('phone_no', code + phone_no);
        // if(userData.phone==""){
        //   formData.append('modal', 14);
        // }
      }
      ...
      if (phone_no == '') {
        ...
        if (userData.phone != code + phone_no) {
          const res = await checkPhoneNo(formData);

          if (res.data.status == false) {
            const confirmation = await signInWithPhone(code + phone_no);
            console.log('myconfirmation', confirmation);
            const auth = userData.auth;
            const authFrom = 'accountinformation';
            const phoneNumber = code + phone_no;
            const fname = first_name;
            const lname = last_name;

            if (confirmation) {
              console.log('myconfirmation1', confirmation);
  ...
  return (
    <SafeAreaView style={{backgroundColor: 'white', height: height}}>
      <ScrollView>
          ...
          leftComponent={
            <View
              ...
              <View>
                ...
                />
                <CustomText
                  title={'Manage your additional information below'}
                  type={'medium'}
                  color={'black'}
        ...
        <View style={{marginTop: -40, paddingRight: 10}}>
          <GradientGoogleButton
            iconRight={arrowright}
            onButtonPress={() => {
              navigation.navigate('Changepassword'); //education & profession
            }}
