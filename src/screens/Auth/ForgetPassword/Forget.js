...
const Forget = ({navigation, signInWithPhone}) => {
  ...
  const handlereset = async () => {
    try {
      if (!phone) {
        setMsg('Kindly Enter Active Phone No to receive OTP');
        setShowAlert(true);
      } else {
        Keyboard.dismiss();
        setLoading(true);
    ...
    <SafeAreaView style={styles.mainContainer}>
      <KeyboardAwareScrollView
        ...
        <View style={{justifyContent: 'center', flex: 0.8}}>
          ...
          />
          <CustomText
            title={'Enter your phone number to reset the process'}
            type={'normal'}
            style={{fontSize: 13, marginLeft: 20, marginTop: 10}}
          />
          <RBSheet
            ref={refRBSheet}
            ...
            <View style={{width: '100%', alignSelf: 'center',marginTop: Platform.OS === 'ios' ? 20: 0}}>
              <TextInput
                style={{
                  width: '100%',
          ...
          {phoneFocus && (
            <CustomText
              title={
                'Kindly Enter Phone no with country code e.g. +921234567890'
              }
              type={'medium'}
              color={'red'}
              style={{fontSize: 9, marginLeft: 20, marginBottom: 20}}
            />
          )}
...
export default connect(null, {signInWithPhone, signup})(Forget);
